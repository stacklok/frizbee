...
func TestReplacer_ParseGitHubActionString(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			args: args{
				action: "actions/setup-go@v5.0.0",
			},
			want: &interfaces.EntityRef{
		...
		},
		{
			...
			args: args{
				action: "actions/setup-go@releases/v5",
			},
			want: &interfaces.EntityRef{
				Name:   "actions/setup-go",
				Ref:    "40f1582b2485089dde7abd97c1529aa768e1baff",
				Type:   actions.ReferenceType,
				Tag:    "releases/v5",
				Prefix: "",
			},
		...
		{
			name: "actions/setup-go with ignored branch returns error",
			args: args{
				action: "actions/setup-go@main",
			},
			wantErr: true,
		},
		{
